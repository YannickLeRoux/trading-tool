...
class StocksScreener implements IStockScreener {
  ...
  bullishEngulfing() {
    ...
    for (let stock of this.dailyData.stocks) {
      ...
      const day1: IDay = Object.values(dataArray[1])[0];
      const day2: IDay = Object.values(dataArray[0])[0];
  ...
  morningStar() {
    ...
    for (let stock of this.dailyData.stocks) {
      const dataArray = Object.values(stock)[0];
      const day1: IDay = Object.values(dataArray[2])[0];
      const day2: IDay = Object.values(dataArray[1])[0];
      const day3: IDay = Object.values(dataArray[0])[0];
      if (
